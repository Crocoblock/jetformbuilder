 ...
 */
addFilter(
	'jet.fb.section.actions.header.register_user',
	'jet-form-builder/register-user',
	function ( prev, action ) {
		const invalid = getInvalidEvents( action?.events ?? [] );
		...
		return <span
			style={ { marginTop: '0px', color: '#cc1818' } }>
		{ __(
			`If you use a Text field type set to "Password," 
			this action will not be successfully executed 
			in the following events: ` + invalid.join( ', ' ),
			'jet-form-builder',
		) }
	</span>;
	},
...
addAction( 'register_user',
	withSelect( withRequestFields )( RegisterUserAction ) );
