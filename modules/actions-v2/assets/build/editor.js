...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! ../helpers/actionRelations */ "./helpers/actionRelations.js");
var _window$JetFBComponen;
...
function ActionItemBody() {
  ...
  const isChild = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_13__.isChildAction)(action);
  ...
  return (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(ActionCardBody, {
    ...
    justify: "flex-start",
    gap: 1
  }, !isChild && (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(CursoredIcon, {
    ...
    justify: "flex-end"
  }, (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(_EditActionSettingsButton__WEBPACK_IMPORTED_MODULE_5__["default"], null), !isChild && (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(react__WEBPACK_IMPORTED_MODULE_0__.Fragment, null, !actionType.disableConditions && (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(_EditActionConditionsButton__WEBPACK_IMPORTED_MODULE_6__["default"], null), (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(_ToggleActionExecutionButton__WEBPACK_IMPORTED_MODULE_7__["default"], null), (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(_DeleteActionButton__WEBPACK_IMPORTED_MODULE_8__["default"], null)))));
}
...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../helpers/actionRelations */ "./helpers/actionRelations.js");
var _window$JetFBComponen;
...
function ActionItemWrapper({
  ...
  return (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(WrapperComponent, {
    ...
    elevation: 2,
    size: 'extraSmall',
    className: (0,_linaria_core__WEBPACK_IMPORTED_MODULE_5__.cx)('jet-form-action', className, !isFixed && !(0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_9__.isChildAction)(action) && 'draggable', !isExecute && disabledStyle, currentAction?.id === action.id && currentStyle, errors.length && errorStyle),
    ...props
  }));
}
...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../../helpers/actionRelations */ "./helpers/actionRelations.js");
...
function AddActionModal() {
  ...
  const onAddAction = (event, action) => {
    ...
    const {
      parent: newAction,
      children
    } = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_6__.createActionGroup)(action, actions);
    setActions([...actions, newAction, ...children]);
...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../helpers/actionRelations */ "./helpers/actionRelations.js");
var _window$JetFBComponen;
...
function EventItem(props) {
  ...
  const isReadOnly = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_7__.isChildAction)(action);
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   RELATION_KEY: () => (/* binding */ RELATION_KEY),
/* harmony export */   createActionGroup: () => (/* binding */ createActionGroup),
/* harmony export */   flattenActionGroups: () => (/* binding */ flattenActionGroups),
/* harmony export */   getActionGroups: () => (/* binding */ getActionGroups),
/* harmony export */   getActionRelation: () => (/* binding */ getActionRelation),
/* harmony export */   isChildAction: () => (/* binding */ isChildAction)
/* harmony export */ });
...
const RELATION_KEY = '_relation';
function getActionRelation(action) {
  var _action$settings$RELA;
  return (_action$settings$RELA = action?.settings?.[RELATION_KEY]) !== null && _action$settings$RELA !== void 0 ? _action$settings$RELA : {};
}
 ...
 */
function createActionGroup(actionType, actions = []) {
  var _actionType$initialSe, _actionType$children;
  const usedIds = new Set(actions.map(action => action.id));
  const parent = new _abstract_BaseAction__WEBPACK_IMPORTED_MODULE_0__["default"]({
    type: actionType.type,
    settings: (_actionType$initialSe = actionType.initialSettings) !== null && _actionType$initialSe !== void 0 ? _actionType$initialSe : {}
  });
  parent.id = createUniqueActionId(usedIds);
  if (actionType.initialSettings) {
    parent.refactorSettings();
  }
  const children = ((_actionType$children = actionType.children) !== null && _actionType$children !== void 0 ? _actionType$children : []).map(childDefinition => {
    const child = new _abstract_BaseAction__WEBPACK_IMPORTED_MODULE_0__["default"](childDefinition);
...
function getActionGroups(actions) {
  const actionsById = new Map(actions.map((action, index) => [action.id, {
    action,
    index
  }]));
  ...
  for (const action of actions) {
    ...
    for (const childId of (_relation$children = relation.children) !== null && _relation$children !== void 0 ? _relation$children : []) {
      var _relation$children;
      childIds.add(childId);
    }
  }
  for (const action of actions) {
    var _getActionRelation$ch;
    if (isChildAction(action) || childIds.has(action.id)) {
      continue;
    }
    const children = ((_getActionRelation$ch = getActionRelation(action).children) !== null && _getActionRelation$ch !== void 0 ? _getActionRelation$ch : []).map(childId => actionsById.get(childId)).filter(child => child && getActionRelation(child.action).parent === action.id);
    groups.push({
      id: action.id,
      items: [actionsById.get(action.id), ...children]
    });
  }

  // Keep malformed/orphaned children visible instead of losing them from UI.
  const groupedIds = new Set(groups.flatMap(group => group.items.map(item => item.action.id)));
  for (const action of actions) {
...
function flattenActionGroups(groups) {
  return groups.flatMap(group => group.items.map(item => item.action));
}
...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../helpers/actionRelations */ "./helpers/actionRelations.js");
var _window$JetFBHooks;
...
function withoutActionBranch(actions, action) {
  ...
  const collectChildren = actionId => {
    ...
    for (const childId of (_getActionRelation$ch = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_1__.getActionRelation)(current).children) !== null && _getActionRelation$ch !== void 0 ? _getActionRelation$ch : []) {
      var _getActionRelation$ch;
  ...
  return actions.filter(current => !idsToDelete.has(current.id)).map(current => {
    const relation = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_1__.getActionRelation)(current);
    if (!relation.children?.some(childId => idsToDelete.has(childId))) {
      return current;
    }
    return {
      ...current,
      settings: {
        ...
        _relation: {
          ...relation,
          children: relation.children.filter(childId => !idsToDelete.has(childId))
        }
      }
    };
  });
...
function getSyncedChild(current, source, props) {
  ...
  const relation = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_1__.getActionRelation)(current);
  const sourceRelation = (0,_helpers_actionRelations__WEBPACK_IMPORTED_MODULE_1__.getActionRelation)(source);
  if (!sourceRelation.children?.includes(current.id) || relation.parent !== source.id) {
    return current;
  }
  const syncedProps = {};
  if (props.hasOwnProperty('events')) {
    var _props$events;
    syncedProps.events = [...((_props$events = props.events) !== null && _props$events !== void 0 ? _props$events : [])];
  }
  if (props.hasOwnProperty('conditions')) {
    var _props$conditions;
    syncedProps.conditions = JSON.parse(JSON.stringify((_props$conditions = props.conditions) !== null && _props$conditions !== void 0 ? _props$conditions : []));
  }
...
/* harmony export */   createActionGroup: () => (/* reexport safe */ _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__.createActionGroup),
/* harmony export */   flattenActionGroups: () => (/* reexport safe */ _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__.flattenActionGroups),
/* harmony export */   getActionGroups: () => (/* reexport safe */ _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__.getActionGroups),
/* harmony export */   getActionRelation: () => (/* reexport safe */ _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__.getActionRelation),
/* harmony export */   globalTab: () => (/* reexport safe */ _helpers_globalTab__WEBPACK_IMPORTED_MODULE_60__["default"]),
/* harmony export */   isChildAction: () => (/* reexport safe */ _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__.isChildAction),
...
/* harmony import */ var _helpers_actionRelations__WEBPACK_IMPORTED_MODULE_61__ = __webpack_require__(/*! ./helpers/actionRelations */ "./helpers/actionRelations.js");
...
var ___CSS_LOADER_EXPORT___ = _node_modules_css_loader_dist_runtime_api_js__WEBPACK_IMPORTED_MODULE_1___default()((_node_modules_css_loader_dist_runtime_sourceMaps_js__WEBPACK_IMPORTED_MODULE_0___default()));
// Module
___CSS_LOADER_EXPORT___.push([module.id, `.ekbdm2s{padding:0 4px;border-radius:5px;color:#5c5c5c;font-size:0.9em;background-color:#f3f4f5;cursor:pointer;margin:auto;border:0;font-family:monospace;}.ekbdm2s:not(:disabled):after{content:' X';font-weight:bold;}.ekbdm2s:disabled{cursor:default;}.ekbdm2s:focus{outline:1px solid #5c5c5c;background-color:#e7e8e9;}
`, "",{"version":3,"sources":["webpack://./components/EventsList.jsx"],"names":[],"mappings":"AAAA,SAAS,aAAa,CAAC,iBAAiB,CAAC,aAAa,CAAC,eAAe,CAAC,wBAAwB,CAAC,cAAc,CAAC,WAAW,CAAC,QAAQ,CAAC,qBAAqB,CAAC,CAAC,8BAA8B,YAAY,CAAC,gBAAgB,CAAC,CAAC,kBAAkB,cAAc,CAAC,CAAC,eAAe,yBAAyB,CAAC,wBAAwB,CAAC","sourcesContent":[".ekbdm2s{padding:0 4px;border-radius:5px;color:#5c5c5c;font-size:0.9em;background-color:#f3f4f5;cursor:pointer;margin:auto;border:0;font-family:monospace;}.ekbdm2s:not(:disabled):after{content:' X';font-weight:bold;}.ekbdm2s:disabled{cursor:default;}.ekbdm2s:focus{outline:1px solid #5c5c5c;background-color:#e7e8e9;}\n"],"sourceRoot":""}]);
// Exports
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (___CSS_LOADER_EXPORT___);
