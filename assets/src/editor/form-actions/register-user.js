...
function RegisterUserAction( props ) {
	...
	useEffect( () => {
		...
		if ( settings.add_user_id ) {
			onChangeSettingObj( { requestFields: [ source.requestFields.user_id ] } )
		}
	}, [] );
	...
	return ( <React.Fragment key="register_user">
		<ToggleControl
			...
			onChange={ allow_register => onChangeSettingObj( { allow_register } ) }
		/>
		{ settings.allow_register && <SelectControl
			...
			onChange={ role_can_register => onChangeSettingObj( { role_can_register } ) }
		/> }
		<BaseControl
