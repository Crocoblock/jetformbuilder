...
- **Various field types.** A total of 18 JetFormBuilder blocks that you can mix and match freely. 
- **Visual editor.** All field types can be found in the same place – the JetForm Fields block.
- **Clear-cut layout.** Use the Column layout to create internal sections and populate them with form fields. 
- **Popular form types.** Build a standard one-column form or go the extra mile and create a multi-step form.
- **Post Submit Actions.** Set the action to be performed after frontend form submission. 
- **Form Break block.** Allows you to break the form into logical steps and add buttons to navigate.
- **Calculated content.** Apply the Calculated field type to perform basic/complex calculations. 
- **Generator Function.** Three available functions that help you generate the field values dynamically.
- **Custom field styles.** JetStyleManager & Gutenberg integration makes it possible to style the form fields anyhow you like. 
