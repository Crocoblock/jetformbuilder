	...
	Base_Module_Static_Instance_It
{
	...
	private function is_gateway_available(string $gateway_id): bool
	{
		if ('' === $gateway_id || 'none' === $gateway_id) {
			return true;
		}

		$registered = array_keys($this->rep_get_items());

		return in_array($gateway_id, $registered, true);
	}
	...
	public function get_form_gateways_by_id($form_id = null)
	{
		...
		$gateway_id = $meta['gateway'] ?? 'none';

		if (!$this->is_gateway_available($gateway_id)) {
			$meta['gateway'] = 'none';
		}
